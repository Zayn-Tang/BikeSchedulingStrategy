...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        query = self.activation(self.q(x))
        key = self.activation(self.k(x))
        value = self.activation(self.v(x))

        query = self.norm(query)
        key = self.norm(key)
        value = self.norm(value)
        
        query = query.reshape(B*T, N, self.embed_dim)
        key = key.reshape(B*T, self.embed_dim, N)
        value = value.reshape(B*T, N, self.embed_dim)
        score = torch.matmul(query, key)
        score = score / self.embed_dim ** 0.5
        score = nn.functional.softmax(score, dim=-1)

        score = torch.matmul(score, value)
        score = score.reshape(B, T, N, self.embed_dim)
        return score
...
class STSAGCN(nn.Module):
    def __init__(self, adj, history, num_of_vertices, in_dim, hidden_dims,
        ...
        self.attention = SpatialAttention(first_layer_embedding_size)
